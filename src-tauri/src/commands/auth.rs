...
pub async fn change_master_password(
    ...
    let result = session
        .with_session_mut(|key_store, vault_store| {
            catch_sync_panic!({
                use crate::crypto::{aes, kdf, kyber};
...
pub async fn create_vault(
    ...
    use crate::crypto::{aes, kdf, keys::ActiveKeyStore, kyber, rng};
    ...
    let mk_raw =
        tokio::task::spawn_blocking(move || kdf::derive_master_key(&pwd_for_kdf, &salt_for_kdf))
            .await
            .map_err(|_| CypheriaError::InternalError("KDF thread panicked".into()))??;
    ...
    let (kyber_ciphertext, vk_wrapped_pq) = kyber::encapsulate_vault_key(&pub_key, &vk_bytes)?;
    let kyber_sk_encrypted = aes::encrypt(&mk_bytes, &sec_key)?;

    let default_settings = Settings::default();
    let settings_json =
        serde_json::to_vec(&default_settings).map_err(|_| CypheriaError::SerdeError)?;
    ...
    let vault_data = VaultData {
        ...
        settings: EncryptedSettings {
            payload_encrypted: settings_encrypted,
        },
        updated_at: Utc::now(),
    };

    let vault_name_encrypted =
        crate::vault::store::encrypt_vault_name(&mk_bytes, &vault_name).unwrap_or_default();
