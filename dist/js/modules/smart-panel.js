...
function buildPrRow(idx, data) {
  ...
  const speakBtn = hasSpeech
    ? `<button class="sp-speak-btn" title="Speak aloud" onclick="spSpeak('${escAttrFull(guide)}')"><svg viewBox="0 0 24 24"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/></svg></button>`
    : '';
  return `
    <div class="sp-row" id="${id}">
      <div class="sp-row-body">
...
window.spPrRegenRow = function (idx) {
...
function renderMnChips(inputIdx) {
  ...
  chipRow.innerHTML = shown.map(w =>
    `<button class="sp-chip" onclick="spMnChip(${inputIdx},'${escAttr(w)}')">${escHTML(w)}</button>`
  ).join('');
}
